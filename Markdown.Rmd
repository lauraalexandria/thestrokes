...
shared_variavel1 <- songs %>% 
  pivot_longer(6:14, names_to = "varx", values_to = "valor") %>% 
  SharedData$new()
shared_variavel2 <- songs %>% 
  pivot_longer(6:14, names_to = "vary", values_to = "valor") %>% 
  SharedData$new()

bscols(widths = c(3, NA),
       
  list(
    filter_select("varx", "Eixo X", shared_variavel1, ~varx, multiple = F),
    filter_select("vary", "Eixo Y", shared_variavel2, ~vary, multiple = F)
  ),
  
  ggplotly( # QUAL O PROBLEMA DO FILTRO??

    ggplot(shared_variavel1, aes(x = valor, color = album_name)) + 
      geom_point(data = shared_variavel2, aes(y = valor))
...
fviz_cluster(k4, geom = "point",  data = data_cluster) + ggtitle("k = 4")

cbind(data_cluster, 
      agrupamento = fitted(k4, method = "class"),
      pca$x[,1:2]) %>% 
  as.data.frame() %>% 
  mutate(across(everything(), as.numeric),
         agrupamento = factor(paste("Grupo", agrupamento), 
                              paste("Grupo", 1:4))) %>% 
  arrange(agrupamento) %>% 
  ...
  e_axis_labels(x = "PCA1 (30.1%)", y = "PCA1 (17.3%)") %>% 
  e_tooltip(
...
agrupamentos <- songs %>% 
  select(2, 6:14) %>% 
  drop_na() %>% 
  mutate(agrupamento = fitted(k4, method = "class"))
